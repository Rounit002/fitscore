...
1. **One accent color per data category, not one color for everything.** In the reference, protein/carbs/fats each get their own icon + their own soft tinted circle badge (a warm red for protein, amber/tan for carbs, blue for fats) instead of reusing a single brand color for every icon regardless of what it represents. Audit every place FitScan shows multiple distinct categories of data next to each other (macros, health metrics, scan categories, whatever exists in the app) and give each category its own consistent, semantically sensible accent + soft-tint badge — the same way the Health Progress stat cards already correctly do this with green/red/blue/amber. Where this pattern is missing (e.g. icon badges that are all the same green regardless of what they represent), add it. This is the "semantic color map" token from Phase 1 — this is what it's for.
...
5. **The reference's date-strip treatment for inactive days (a light dotted/thin circle outline, versus a bold solid ring on the selected day) is a good real-world example of exactly the "hairline outline vs. bold outline" split already decided in Phase 1.** Use it as the concrete reference when applying that rule to FitScan's own date strip and anywhere else a similar selected/unselected set of items appears (tabs, filter chips, day/week selectors).
